...
%% Plot VPol Data
figure; 
set(gcf, Units="centimeters", Position=[3 3 fig_width fig_height]);
...
for i=1:length(VPol_file_list)
    ...
    [~, idx] = min(abs(VPol_freq - target_freq)); 
    freq = VPol_freq(idx); 
...
for i=1:length(HPol_file_list)
    ...
    % Find the closest frequency to the target
    [~, idx] = min(abs(HPol_freq - target_freq)); 
    freq = HPol_freq(idx); 
...
sgtitle(freq + " GHz Patterns", FontName=fontname, FontSize=fontsize_sgtitle, FontWeight="bold");

title("Cross-Pol (H-Pol)");
xlabel("Angle (°)");
ylabel("S_{21} (dB)", Interpreter="tex");
